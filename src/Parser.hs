...
happyExpList = Happy_Data_Array.listArray (0,40) ([2048,16384,0,4,0,8192,0,1,6,0,384,4352,34816,0,0,34,0,0,0,24576,2,0,152,0,9728,0,0,8,0,0,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseQDIMACS","Problem","QBF","QuantifierSet","ClauseSet","Clause","AtomSet","Atom","Literal","p","cnf","'0'","'-'","a","e","int","%eof"]
        bit_start = st * 19
        bit_end = (st + 1) * 19
...
action_18 (14) = happyShift action_21
...
happyReduction_1 ((HappyAbsSyn5  happy_var_5) `HappyStk`
	(HappyTerminal (L.TokenInt happy_var_4)) `HappyStk`
	(HappyTerminal (L.TokenInt happy_var_3)) `HappyStk`
	_ `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn4
		 (Problem happy_var_3 happy_var_4 happy_var_5
	) `HappyStk` happyRest
...
happyReduction_2 (HappyAbsSyn7  happy_var_3)
	...
	(HappyAbsSyn6  happy_var_1)
	 =  HappyAbsSyn5
		 (QBF (happy_var_1,happy_var_2) happy_var_3
	)
...
happyReduction_10 (HappyAbsSyn9  happy_var_2)
	(HappyAbsSyn10  happy_var_1)
	 =  HappyAbsSyn9
		 (Set.insert happy_var_1 happy_var_2
	)
happyReduction_10 _ _  = notHappyAtAll 

happyReduce_11 = happySpecReduce_1  10 happyReduction_11
happyReduction_11 (HappyTerminal (L.TokenInt happy_var_1))
	 =  HappyAbsSyn10
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	L.TokenCNF -> cont 13;
	L.TokenZero -> cont 14;
	L.TokenMinus -> cont 15;
	L.TokenA -> cont 16;
	L.TokenE -> cont 17;
	L.TokenInt happy_dollar_dollar -> cont 18;
	_ -> happyError' ((tk:tks), [])
	}
...
parseQDIMACS tks = happyRunIdentity happySomeParser where
